...
<body>
            ...
            $stmt = $conn->prepare("SELECT * FROM ouvrage WHERE id_ouvrage  IN (SELECT id_ouvrage  FROM réservations WHERE id_adhérent =:id_adhérent  and reservation_valid = 0)");
            $stmt->bindParam(':id_adhérent ', $id);
            
            $ouvrages = $stmt->fetchAll(PDO::FETCH_ASSOC);

            
                foreach ($ouvrages as $ouvrage) {
                    $stmt = $conn->prepare("SELECT * FROM réservations WHERE id_adhérent =6 AND id_ouvrage =3 ");
                    ...
                    <div class='book-card-content'>
                      <h3 class='book-card-title'> $ouvrage[titre]</h3>
                      <p class='book-card-author'>$ouvrage[auteur]</p>
                      <p class='book-card-info'>
                        <span class='book-card-type' >$ouvrage[type]</span>
                        <span class='book-card-status'>$ouvrage[date_achat]</span>
                        <span class='book-card-edition'>$ouvrage[description]</span>
                      </p>
