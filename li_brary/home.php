            ...
            <div class="menuBar">
                ...
                <ul>
                    ...
                    <li>
                        ...
                        <a href="php\adhérents_reservation.php">réservations</a>
                    </li>
        ...
        <div class="main" id="main">
            ...
            <div class="offer" id="offer">
                <div class="card">
                    ...
                    <input name="submit" class=' btn-p' type="submit" value="Search">
                </div>
...
<?php
            echo "<input type='hidden' name='book_id' value='<?php echo $id ?>'>";
        $searchEtat = isset($_GET['etat']) ? $_GET['etat'] : '';
   if (empty($searchKeyword) && $searchEtat == 'all' && isset($_GET["submit"])) {
        $searchKeyword = isset($_GET['search']) ? $_GET['search'] : '';

        $stmt = $conn->prepare("SELECT * FROM ouvrage WHERE titre LIKE :searchKeyword");
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "<img src='$row[image_couverture]' width='90px' height='90px' alt=''>";
            echo  "<h1 class='itemTittle'>$row[titre]</h1>";
            echo  "<p>$row[description]</p>";
            echo  "<span class='price'> <a href='php/details.php?id=$row[id_ouvrage]' class='btn btn-primary'>reservation</a></span>";
            echo "</div>";
        }
        ...
        $sql = "SELECT * FROM ouvrage WHERE titre LIKE :searchKeyword";
        if (!empty($searchEtat)) {
            $sql .= " AND etat = :searchEtat";
        }

        $stmt = $conn->prepare($sql);
        $stmt->bindValue(':searchKeyword', '%' . $searchKeyword . '%', PDO::PARAM_STR);
        if (!empty($searchEtat)) {
            $stmt->bindValue(':searchEtat', $searchEtat, PDO::PARAM_STR);
        }
        $stmt->execute();
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo  "<h1 class='itemTittle'>$row[titre]</h1>";
                echo  "<p>$row[description]</p>";
                echo  "<span class='price'> <a href='php/details.php?id=$row[id_ouvrage]' class='btn btn-primary'>Voir Plus</a></span>";
                echo "</div>";
            }
